...
// Actor Stats
constexpr s32 hp = 12;
constexpr s32 dmgNormalElectroDash = 3;
constexpr s32 dmgSuperElectroDash = 4;
constexpr s32 dmgUltraElectroDash = 5;
...
API_CALLABLE(WattFXUpdate) {
    ...
    if (isInitialCall) {
         wattEffectData = (WattEffectData*)heap_malloc(sizeof(*wattEffectData));
    ...
    wattEffectData = (WattEffectData*)state->varTablePtr[2];
...
API_CALLABLE(WattFXRemove) {
    WattEffectData* wattEffectData = (WattEffectData*)get_actor(script->owner1.enemyID)->state.varTablePtr[2];
...
API_CALLABLE(WattFXSetBouncing) {
...
EvtScript EVS_HandleEvent = {
    ...
    Call(WattFXSetBouncing, 0)
    Call(WattFXSetActive, 1)
...
EvtScript EVS_TakeTurn = {
    ...
    Loop(LVarA)
        ...
        IfGt(LVar9, 200)
            Set(LVar9, 200)
        EndIf
        Call(watt_clone::SetBackgroundAlpha, LVar9)
        Wait(1)
    EndLoop
    Call(SetAnimation, ACTOR_SELF, PRT_MAIN, ANIM_BattleWatt_Idle)
    ...
    Thread
        Wait(2)
        Call(watt_clone::SetBackgroundAlpha, 0)
    EndThread
...
#else
            Call(EnemyDamageTarget, ACTOR_SELF, LVar0, DAMAGE_TYPE_SHOCK | DAMAGE_TYPE_IGNORE_DEFENSE, SUPPRESS_EVENT_ALL, 0, dmgNormalElectroDash, BS_FLAGS1_TRIGGER_EVENTS)
#endif
...
#else
            Call(EnemyDamageTarget, ACTOR_SELF, LVar0, DAMAGE_TYPE_SHOCK | DAMAGE_TYPE_IGNORE_DEFENSE, SUPPRESS_EVENT_ALL, 0, dmgSuperElectroDash, BS_FLAGS1_TRIGGER_EVENTS)
#endif
...
#else
            Call(EnemyDamageTarget, ACTOR_SELF, LVar0, DAMAGE_TYPE_SHOCK | DAMAGE_TYPE_IGNORE_DEFENSE, SUPPRESS_EVENT_ALL, 0, dmgUltraElectroDash, BS_FLAGS1_TRIGGER_EVENTS)
#endif
    EndSwitch
    Switch(LVar0)
