...
// Actor Stats
constexpr s32 hp = 12;
constexpr s32 dmgNormalHeadbonk = 1;
constexpr s32 dmgSuperHeadbonk = 2;
constexpr s32 dmgUltraHeadbonk = 3;
    ...
    Switch(LVar0)
        ...
        CaseOrEq(EVENT_HIT)
            ...
            IfFlag(LVar1, DAMAGE_TYPE_SHOCK)
                ...
                ExecWait(duplighost::EVS_Duplighost_OnHitElectric)
                Return
            Else
                SetConst(LVar0, PRT_MAIN)
                SetConst(LVar1, ANIM_BattleGoombario_HurtStill)
        ...
        CaseEq(EVENT_SHOCK_HIT)
            SetConst(LVar0, PRT_MAIN)
            SetConst(LVar1, ANIM_BattleGoombario_HurtStill)
            ExecWait(duplighost::EVS_Duplighost_OnShockHit)
            Return
        CaseEq(EVENT_SHOCK_DEATH)
            ...
            SetConst(LVar1, ANIM_BattleGoombario_HurtStill)
            Set(LVar2, 12)
            ExecWait(duplighost::EVS_Duplighost_OnShockDeath)
            SetConst(LVar0, PRT_MAIN)
            SetConst(LVar1, ANIM_BattleGoombario_HurtStill)
            ExecWait(EVS_Enemy_Death)
            Return
...
EvtScript EVS_HeadbonkFollowthrough = {
...
EvtScript EVS_CalculateJumpTime = {
    Call(SetGoalToTarget, ACTOR_SELF)
...
EvtScript EVS_Attack_Headbonk = {
    Call(UseIdleAnimation, ACTOR_SELF, false)
    Call(EnableIdleScript, ACTOR_SELF, IDLE_SCRIPT_DISABLE)
    ExecWait(EVS_ApproachPlayer)
    ExecWait(EVS_CalculateJumpTime)
    Call(GetStatusFlags, ACTOR_SELF, LVarA)
    IfFlag(LVarA, STATUS_FLAG_SHRINK)
    ...
    Switch(LVar0)
        ...
        CaseOrEq(HIT_RESULT_LUCKY)
            ...
            Wait(20)
            Call(YieldTurn)
            ExecWait(EVS_HeadbonkFollowthrough)
            Call(EnableIdleScript, ACTOR_SELF, IDLE_SCRIPT_ENABLE)
            Call(UseIdleAnimation, ACTOR_SELF, true)
            Return
        EndCaseGroup
    ...
    Switch(LVar9)
        CaseEq(PARTNER_RANK_NORMAL)
            Call(EnemyDamageTarget, ACTOR_SELF, LVar0, 0, 0, 0, dmgNormalHeadbonk, BS_FLAGS1_INCLUDE_POWER_UPS)
        CaseEq(PARTNER_RANK_SUPER)
            Call(EnemyDamageTarget, ACTOR_SELF, LVar0, 0, 0, 0, dmgSuperHeadbonk, BS_FLAGS1_INCLUDE_POWER_UPS)
        CaseEq(PARTNER_RANK_ULTRA)
            Call(EnemyDamageTarget, ACTOR_SELF, LVar0, 0, 0, 0, dmgUltraHeadbonk, BS_FLAGS1_INCLUDE_POWER_UPS)
    EndSwitch
...
API_CALLABLE(CalculateTattleCamBoomLength) {
...
EvtScript EVS_Move_Tattle = {
    ...
    Sub(LVar0, 8)
    Set(LVar1, 0)
    Call(CalculateTattleCamBoomLength)
    Wait(1)
