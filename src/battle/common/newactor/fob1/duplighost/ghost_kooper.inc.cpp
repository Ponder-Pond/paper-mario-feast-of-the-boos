...
// Actor Stats
constexpr s32 hp = 12;
constexpr s32 dmgNormalShellToss = 2;
constexpr s32 dmgSuperShellToss = 3;
constexpr s32 dmgUltraShellToss = 5;
...
EvtScript EVS_HandleEvent = {
    ...
    Switch(LVar0)
        ...
        CaseEq(EVENT_FLIP_TRIGGER)
            ...
            Call(SetIdleAnimations, ACTOR_SELF, PRT_MAIN, Ref(ToppleAnims))
            Call(SetTargetOffset, ACTOR_SELF, PRT_MAIN, 5, 15)
            Call(SetProjectileTargetOffset, ACTOR_SELF, PRT_MAIN, 0, -6)
...
#else
            Call(EnemyDamageTarget, ACTOR_SELF, LVar0, 0, SUPPRESS_EVENT_ALL, 0, dmgNormalShellToss, BS_FLAGS1_TRIGGER_EVENTS)
#endif
        CaseEq(PARTNER_RANK_SUPER)
            Wait(2)
            Call(EnemyDamageTarget, ACTOR_SELF, LVar0, 0, SUPPRESS_EVENT_ALL, 0, dmgSuperShellToss, BS_FLAGS1_TRIGGER_EVENTS)
        CaseEq(PARTNER_RANK_ULTRA)
            Wait(2)
#if VERSION_JP
            Call(EnemyDamageTarget, ACTOR_SELF, LVar0, 0, SUPPRESS_EVENT_ALL, 0, 4, BS_FLAGS1_TRIGGER_EVENTS)
#else
            Call(EnemyDamageTarget, ACTOR_SELF, LVar0, 0, SUPPRESS_EVENT_ALL, 0, dmgUltraShellToss, BS_FLAGS1_TRIGGER_EVENTS)
#endif
    EndSwitch
    Switch(LVar0)
